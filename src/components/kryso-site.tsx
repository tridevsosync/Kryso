...
export function SiteShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className="min-h-screen bg-background text-foreground">
    ...
    <Dialog open={enquiryOpen} onOpenChange={(open) => { setEnquiryOpen(open); if (!open) setSubmitted(false); }}><DialogContent className="max-h-[90vh] overflow-y-auto rounded-2xl"><DialogHeader><DialogTitle className="font-display text-2xl">Let’s get you started</DialogTitle><DialogDescription>Tell us what you’d love to learn. We’ll help you find your rhythm.</DialogDescription></DialogHeader>{submitted ? <div className="rounded-xl bg-accent p-6 text-center"><span className="text-3xl">♫</span><p className="mt-3 font-semibold">Thank you! Your enquiry is ready.</p><p className="mt-1 text-sm text-muted-foreground">Our team will be in touch soon.</p></div> : <form className="grid gap-4" onSubmit={(event) => { event.preventDefault(); setSubmitted(true); }}><label className="grid gap-1.5 text-sm font-semibold">Your name<Input required placeholder="Name"/></label><label className="grid gap-1.5 text-sm font-semibold">Phone number<Input required type="tel" placeholder="+91"/></label><label className="grid gap-1.5 text-sm font-semibold">Email<Input type="email" placeholder="you@example.com"/></label><label className="grid gap-1.5 text-sm font-semibold">What would you like to learn?<select className="h-10 rounded-md border border-input bg-background px-3 font-normal" defaultValue=""><option value="" disabled>Select a course</option>{["Guitar", "Piano", "Keyboard", "Drums", "Violin", "Flute", "Harmonium", "Tabla", "Vocal singing"].map((name) => <option key={name}>{name}</option>)}</select></label><label className="grid gap-1.5 text-sm font-semibold">A little more about it<Textarea placeholder="Your message" rows={3}/></label><Button type="submit" className="h-11 rounded-full font-bold">Send enquiry <ArrowUpRight size={16}/></Button><p className="text-center text-xs text-muted-foreground">Demo form · no information is sent or stored.</p></form>}</DialogContent></Dialog>

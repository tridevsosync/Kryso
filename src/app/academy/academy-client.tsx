...
const perks = [
  { icon: Award, title: "Experienced teachers", text: "Learn with working musicians who love to teach." },
  { icon: Heart, title: "A friendly room", text: "Start at your own pace, with no experience needed." },
  { icon: UsersRound, title: "Personal attention", text: "Small group and one-to-one learning, your way." },
  { icon: Music2, title: "Make music together", text: "Practice sessions and chances to play for an audience." },
    ...
    <SiteShell>
        ...
        <div className="page-shell grid gap-12 py-16 md:grid-cols-[1.1fr_.9fr] md:items-center md:py-20">
          <div>
            ...
            <h1 className="mt-4 font-display text-4xl font-extrabold leading-tight sm:text-5xl">
              A little music can change <span className="text-primary">a lot.</span>
            </h1>
            <p className="mt-5 max-w-xl leading-7 text-secondary-foreground/70">
              We make learning music welcoming, practical and full of those small moments that make you want to keep
              playing.
            </p>
            <EnquiryButton className="mt-8 h-12 rounded-full px-6 font-bold">
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              { word: "Perform", line: "when you're ready" },
            ].map((item, i) => (
              <div
                key={item.word}
                className={`min-h-32 p-5 ${
                  i === 1 || i === 2 ? "bg-primary text-primary-foreground" : "bg-secondary-foreground/10"
                }`}
              >
                <span className="font-display text-2xl font-extrabold">{item.word}</span>
                <p className="mt-2 text-sm opacity-75">{item.line}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="page-shell section-space grid gap-12 lg:grid-cols-2">
        ...
        <div className="grid content-start gap-6 border-l-2 border-primary pl-6">
          <div>
            ...
            <p className="mt-2 font-display text-xl font-bold">Make music education joyful, accessible and personal.</p>
          </div>
          <div>
            <p className="eyebrow">Our vision</p>
            <p className="mt-2 font-display text-xl font-bold">A community where everyone feels at home with music.</p>
          </div>
          <div>
      ...
      <section className="page-shell section-space">
        ...
        <div className="mt-9 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {courses.map((course) => (
            <article key={course.id} className="border border-border bg-card p-6">
              <div className="flex items-center justify-between">
                <div className="grid size-12 place-items-center rounded-full bg-accent font-display text-xl text-primary">
                  {course.icon}
                </div>
                <span className="text-xs font-semibold text-muted-foreground">{course.level}</span>
              </div>
              <h2 className="mt-5 font-display text-xl font-bold">{course.name}</h2>
              ...
              <div className="mt-5 grid grid-cols-2 gap-3 border-t border-border pt-4 text-xs">
                <div>
                  ...
                  <span className="mt-1 block font-bold">{course.duration}</span>
                </div>
                <div>
                  <span className="block text-muted-foreground">Monthly fees</span>
                  <span className="mt-1 block font-bold">₹{course.fees.toLocaleString("en-IN")}</span>
                </div>
              </div>
              <p className="mt-4 text-xs text-muted-foreground">With {course.instructor}</p>
              <EnquiryButton variant="outline" className="mt-5 h-10 w-full rounded-full">
      ...
      <section className="bg-accent/35">
        <div className="page-shell section-space grid gap-12 md:grid-cols-[.8fr_1.2fr] md:items-center">
          ...
          <div className="bg-card p-7 sm:p-10">
            ...
            <p className="mt-5 font-display text-xl font-semibold leading-8">“{testimonials[review]?.text}”</p>
            <div className="mt-7 flex items-center gap-3 border-t border-border pt-5">
              <span className="grid size-11 place-items-center rounded-full bg-accent font-display font-bold text-primary">
                {testimonials[review]?.name.charAt(0)}
              </span>
              <div>
                <p className="font-bold">{testimonials[review]?.name}</p>
      ...
      <section className="page-shell py-16">
        <div className="grid gap-6 sm:grid-cols-4">
          {["In the practice room", "Learning together", "Sharing a first song", "Music in the making"].map(
            (caption, i) => (
              <div
                key={caption}
                className={`grid aspect-[4/3] place-items-end p-4 ${
                  ["bg-accent/60", "bg-muted", "bg-secondary text-secondary-foreground", "bg-primary/20"][i]
                }`}
              >
      ...
      <section className="bg-secondary text-secondary-foreground">
        <div className="page-shell flex flex-wrap items-center justify-between gap-6 py-12">
          <div>
            ...
            <h2 className="mt-2 font-display text-2xl font-extrabold">Your next favourite thing could be a song.</h2>
          </div>
